...
object RuleEngine {
  ...
  val equiRules = new MutableList[EquiRule]
  val generalRules = new MutableList[GeneralRule]
  val insertionRules = new MutableList[InsertionRule]
  ...
  def headConforms(headA: Head, headB: Head): Boolean = {
    ...
    if(headA.typo == headB.typo) {
      ...
      var specA: Spec = null
      var specB: Spec = null
      while(i < equiRules.size && conformed == false) {
        specA = equiRules.apply(i).getEquiSpec(headA)
        j = 0
        while(j < equiRules.size && conformed == false) {
  ...
  def performFitting(head: Head, spec: Spec): Boolean = {
    ...
    var fits = (if(!spec.forms.isEmpty) spec.forms.contains(head.form) else true) && (if(spec.typo != Head.UNDEF) fitTypo else true)
//    if(head.typo == Head.CJP)
...
//      println(s"fit (after): fits=${fits}, ${spec} vs ${head}")
  ...
  def absolveByGeneralRules(a: Phrase, b: Phrase): MutableList[Phrase] = {
    var productPhrases = new MutableList[Phrase]
    var i = 0
...
class EquiRule(desc: String) {
  ...
  val rule = RuleParser.parseOne(desc)
  val equiA = rule.body.head
  val equiB = rule.product
...
//        println(s"equirule.fit: name: ${pair._1}, value: ${pair._2}, peer: ${head.read(pair._1)}")
    ...
    var equiBFits = spec.typo == equiB.typo
...
class GeneralRule(desc: String) {
  ...
  val rule = RuleParser.parseOne(desc)
  rule.finish
  val specA = rule.body.apply(0)
  val specB = rule.body.apply(1)
  
  def produce(a: Phrase, b: Phrase): MutableList[Phrase] = {
    if(a.coreHead != null && b.coreHead != null && RuleEngine.fit(a.coreHead, specA) && RuleEngine.fit(b.coreHead, specB)) {
      ...
      val map = new HashMap[String, Head]
      if(specA.id != null)
      ...
      map.foreach(pair => {
        ...
        if(spec != null && !spec.operations.isEmpty) {
...
//        productPhrases
        ...
        (new MutableList[Phrase]) += productPhrase
      }
    }
    else
      null
...
class InsertionRule(desc: String) {
  ...
  def produce(insertee: Phrase, inserter: Head) {
    ...
    if(!rule.product.operations.isEmpty) {

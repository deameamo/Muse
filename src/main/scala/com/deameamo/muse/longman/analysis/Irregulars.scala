...
object Irregulars {
  ...
  private val map = new HashMap[String, MutableList[Morph]]
  ...
  def getPluralForm(form: String, irregulars: MutableList[Irregular]): String = {
    if(irregulars.isEmpty)
      form
    else {
      val plurals = new MutableList[String]
  ...
  def getVerbForm(form: String, irregulars: MutableList[Irregular], hwd: String, function: String, tense: String, personNumber: String): String = {
    if(hwd == "be") {
      val correct = {
      ...
      if(irregulars.isEmpty)
        ...
        val corrects = new MutableList[String]
        irregulars.foreach(irregular => {
  ...
  private def addIntoMap(form: String, morph: Morph) {
    if(!map.contains(form)) {
      map.put(form, new MutableList[Morph])
    }
    map.apply(form) += morph
  }
  
  def get(form: String, pos: String, originality: String): MutableList[Morph] = {
